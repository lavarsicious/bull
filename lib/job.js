 ...
 * A timeout can be enforced.
 */
Job.prototype.finished = async function(beforeRetry, timeout, watchdogInterval = FINISHED_WATCHDOG) {
  await Promise.all([
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkAbort = async () => {
        ...
        if (this._isQueueClosing()) {
          throw new Error('cannot check if job is finished in a closing queue.');
        }
        ...
        if (typeof beforeRetry === 'function') {
          ...
          if (!result) {
            throw new Error('request to check if job is finished has been aborted.')
          }
        }
      };

      const watchdog = () => checkAbort()
        .then(() =>
      ...
      // Watchdog
      //
      const interval = setInterval(watchdog, watchdogInterval);
    });
  }
};
...
Job.prototype._saveAttempt = function(multi, err) {
  ...
  multi.saveAttempt(this.queue.toKey(this.id), this.lockKey(), this.queue.token, ...Object.entries(params).flat());
};
